...
interface SavingsModalProps {
    onClose: () => void;
    onSave: (savingData: { amount: number; month: number; year: number; }) => void;
    currentAmount?: number;
    month: number;
    year: number;
}

const SavingsModal: React.FC<SavingsModalProps> = ({ onClose, onSave, currentAmount, month, year }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="w-full max-w-md bg-surface backdrop-blur-xl p-8 rounded-xl shadow-neu-lg border-t border-l border-b border-r border-t-border-highlight border-l-border-highlight border-b-border-shadow border-r-border-shadow relative">
                ...
                <h2 className="text-2xl font-bold text-text-primary mb-2 text-center">Set Monthly Savings</h2>
                <p className="text-text-secondary text-center mb-6">for {selectedMonthName}</p>
                
                <form className="space-y-6" onSubmit={handleSubmit}>
                    <div>
                        <label htmlFor="amount" className="block text-sm font-medium text-text-secondary mb-1">Total Savings Amount</label>
